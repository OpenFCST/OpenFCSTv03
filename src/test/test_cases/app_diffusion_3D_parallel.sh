...
############################################################################################################
cd ../data/app_diffusion/testing/3D_Parallel

rm solution.dat

mpirun -np 4 ../../../../bin/fuel_cell-3d.bin main.prm
echo  "called!"
if [ "${PIPESTATUS[0]}" != "0" ]; then
    echo  2>&1 | tee tests_summary.log
    echo  "Results summary from the app_diffusion_3D_parallel test:" 2>&1 | tee --append tests_summary.log
...
python ../../../../test/simpleCompare.py solution_ref.dat solution.dat
    if [ "${PIPESTATUS[0]}" != "0" ]; then
        ...
        echo "Please check the results in solution.dat against that of solution_ref.dat" 2>&1 | tee --append tests_summary.log
        echo "		" 2>&1 | tee --append tests_summary.log	
        echo "IMPORTANT: This test runs the code in parallel. A version of openMPI should be implemented in" 2>&1 | tee --append tests_summary.log			
        echo "           your computer and the command mpirun should work" 2>&1 | tee --append tests_summary.log
        echo "		" 2>&1 | tee --append tests_summary.log		
        echo "IMPORTANT: This test will only pass if openFCST is configured using --with-petsc option" 2>&1 | tee --append tests_summary.log		
        echo "-------------------------------------------------------------------------------" 2>&1 | tee --append tests_summary.log
        echo 2>&1 | tee --append tests_summary.log
        cat tests_summary.log >> ../../../../tests_summary.log
        exit 2
        ...
        echo 2>&1 | tee --append tests_summary.log
        echo "--------------------------------------------" 2>&1 | tee --append tests_summary.log
        echo "Results from the test match expected results. Your parallel implementation is working properly." 2>&1 | tee --append tests_summary.log
        echo  "--------------------------------------------" 2>&1 | tee --append tests_summary.log
        echo 2>&1 | tee --append tests_summary.log
        cat tests_summary.log >> ../../../../tests_summary.log
        exit 0
