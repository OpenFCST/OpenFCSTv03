...
test_function_polarization () {

test_name=$1
test_script="$2/test/csvPoleCurveCompare.py"
...
if [ "$error_status" != "0" ]; then
    ...
    python $test_script polarization_curve.dat ./regression/test_results.dat
    error_status=${PIPESTATUS[0]} 
    if [ "$error_status" != "0" ] && [ "$3" = "NoPETSc" ]; then
        echo                                                                                   2>&1 | tee          tests_summary.log
        echo "Results summary from the $test_name test:"                                2>&1 | tee --append tests_summary.log
        echo                                                                                   2>&1 | tee --append tests_summary.log
        echo "-------------------------------------------------------------------------------" 2>&1 | tee --append tests_summary.log
